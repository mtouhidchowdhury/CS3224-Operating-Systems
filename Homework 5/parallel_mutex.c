...
int keys[NUM_KEYS];
pthread_mutex_t lock; //for part 1
pthread_mutex_t mutexes[NUM_BUCKETS]; //part 4
...
void insert(int key, int val) {
    ...
    pthread_mutex_lock(&mutexes[i]);//part 4
    ...
    pthread_mutex_unlock(&mutexes[i]);//part 4
    /*part 3: no changes needed from part 1, since keys are already
...
int main(int argc, char **argv) {
    ...
    pthread_t *threads;
    double start, end;
    //pthread_mutex_init(&lock, NULL); // for part 1
    for (i = 0; i < NUM_BUCKETS; i++){
        pthread_mutex_init(&mutexes[i], NULL); //part 4
    }
