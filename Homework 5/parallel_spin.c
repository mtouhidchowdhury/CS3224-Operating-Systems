...
void insert(int key, int val) {
    pthread_spin_lock(&spinlock);//part 2
    ...
    e->val = val;
    table[i] = e;
    pthread_spin_unlock(&spinlock);//part 2

/*

Part 2:
parallel_mutex, it took 0.00677 secs to insert 100000 keys with 1 thread and it took 8.4475 secs 
to retreive the keys.

parallel_mutex, it took 0.01211 secs to insert 100000 keys with 8 thread and it took 2.4667 secs 
to retrieve the keys.

parallel_spin, it took 0.00566 secs to insert 100000 keys with 1 thread and it took 8.9772 secs 
to retrieve the keys.

parallel_spin, it took 0.01773 secs to insert 100000 keys with 8 thread and it took 2.3673 secs 
to retrieve the keys.

parallel_mutex took longer to insert all keys . So it seem that for inserts parallel_spin is quicker 
while for retrieves parallel_mutex is quicker. The timings were closer than expected
*/
}
...
int main(int argc, char **argv) {
    ...
    pthread_t *threads;
    double start, end;
    pthread_spin_init(&spinlock, 0);// part 2
